...
## 260813 (계속) — Supplier_Contacts 이름 미확보 3건(SC-002/004/017) 보강

**배경:** Claude Browser(비로그인)로 SC-002/004/017의 FB 프로필 URL 확인을 시도했으나 전부 로그인 페이지로 리다이렉트돼 실패("계속하려면 로그인해주세요") — 로그인된 세션 없이는 FB 그룹 회원 프로필에 접근 불가함을 확인. 회장이 본인 브라우저(로그인 상태)로 직접 3개 URL을 열어 스크린샷 3장을 제공.

**실행:** 스크린샷에서 확인된 이름·소개글을 `Supplier_Contacts`에 반영(Airtable Write, 코드 변경 없음).
- SC-002(`100055940777672`) → name=**Ho Sung Jung**
- SC-004(`61556148705736`) → name=**David Ko Koreacosmetics**, notes에 소개글 추가("Korean Cosmetics Wholesale Supplier 한국 화장품 도매. WA/TG 있다고 표기, 구체 ID는 미기재, 카카오만 확인됨")
- SC-017(`100030564422432`) → name=**이용균**, notes에 소개글 추가("마스크팩 선크림 영양크림 아이크림 기초세트 제조 공장, OEM 생산 가능")

**상태변경 총계:** Airtable Write 1건(3개 레코드 name/notes 필드만 갱신). 코드 변경 없음.

**잔여 과제:** 없음 — Supplier_Contacts 22건 전부 최소 URL 이상 확보, 이 중 이름 미확보는 이제 0건(연락처 채널 자체가 없는 항목은 여전히 일부 남아있음, 회장 추후 보강 예정 범위 내).
...
## 260814 (계속) — ERR-113 재발(실제 자동 슬롯 08:00/11:00) + 상한 재정의로 2차 완화

**배경:** 위 항목의 Commit `cae64d5`(1차 완화, 42/20/26 등) 이후 회장이 "하루5번인데 1번만 게시됨" 보고 — 실제로 12:41 ICT 기준 05:00만 성공, 08:00/11:00 슬롯 확인 필요.

**1) Read-only 확인:** `logs/summary/app.log`에서 오늘(260814) 실제 슬롯 결과 직접 대조 — 05:00 Producer 성공(`record_id=recTWdO4UclyS17hk`)+06:00 Publish 성공. 08:00 Producer `[ERROR] 콘텐츠 생성 실패 | error_code=IMAGE_GENERATION_FAILED`, 09:00 Publish는 "executed successfully"지만 ready 레코드가 없어 실질적으로 no-op. 11:00도 동일 패턴. `[HeroCardImage]` 진단 로그로 08:00=`SUBHEADLINE_INVALID`, 11:00=`TAGLINE_INVALID` 확정 — **1차 완화(42/26)로도 실제 운영에서 재발**.

**2) 재정의 + 2차 완화:** "실측값+여유"로 상한을 정하는 접근 자체가 한계임을 인정 — `render_hero_card()`가 이제 모든 텍스트 요소(headline/subheadline/tagline/block)에 폰트 자동축소 안전장치를 갖췄으므로, 상한을 "렌더링 안전을 위한 것"에서 "명백히 잘못된 응답만 거르는 최후 방어선"으로 재정의하고 대폭 상향(2배 이상). 신규 topic 4개로 재검증 4/4(100%) 확인.

**3) 문서 갱신:** `docs/ERROR_DATABASE.md`(ERR-113에 "추가 1"/"추가 2" 단락 보강), `docs/FAILURE_PATTERN.md`(FP-084 예방 항목 재작성 — "재현 검증 성공"과 "완전 해소"를 구분해야 한다는 교훈 반영), `docs/VALIDATION_STATUS.md`(신규 1행), 이 항목.

**상태변경 총계:** 코드 변경 2건(`hero_card_content_builder.py` 상한값, 테스트 fixture). Airtable Write·`.env`·Runtime 재시작 0건(이번 항목 범위 — 코드만 수정, 다음 실제 슬롯이 자동으로 새 코드 적용받음, 별도 재시작 불요 — 서비스가 매 슬롯마다 코드를 새로 읽는 방식이 아니라 이미 실행 중인 프로세스가 계속 도는 구조이므로, **정확히는 다음 서비스 재시작 이후부터 반영**된다는 점을 다음 세션에서 재확인 필요).

**잔여 과제(다음 세션 또는 이 세션 계속):** (1) 이 커밋이 실제 서비스에 반영되려면 재시작이 필요할 수 있음(ERR-109 교훈 재적용 여부 확인 필요) — 14:00/17:00 슬롯 결과로 간접 확인 가능하나 확실히 하려면 재시작 권장. (2) 14:00/17:00 슬롯 결과 육안 확인. (3) hero card fingerprint dedup(P2) 여전히 미해결.
...
commit: `48c4116` (feat, 14파일) — **정정(260917)**: 작성 당시 '진행 예정'으로 남아 있었으나 실제로는 커밋 완료
push: 완료 — 260917 `git merge-base --is-ancestor 48c4116 origin/master` 로 원격 포함 확인

---

## 260905~260907 — STEP 3-G 일일 운영 검증: 8건 → 8건 → 10건(상한 최초 도달) (기록 260917 백필)

**배경:** 260904 scheduler ON 이후 실제 일일 실적으로 "하루 10건 무인운영"을 검증한 구간. 당시 세션에서 결과 보고와 일부 커밋(`dff82e3`/`918269b`)까지 했으나 이 저널에는 260905 이후가 누락돼 260917에 작업일 기준으로 백필한다. 이미 push된 커밋 이력은 수정하지 않는다.

**1) 260905 — 8건 (ERR-121):** 오전 3시간 손실로 상한 미달. ①절전 — 충전기가 전원으로 인식되지 않음(`PowerOnline=False`) + 배터리 절전 타이머 `STANDBYIDLE DC=180초`, 회장 조치 후 `PowerOnline=True`·`AC=0/DC=0` 확인(※ DC=0 규칙은 이후 ERR-134(260917)로 폐기, AC 상시연결이 유일 원칙) ②09:02 회차가 FB 크롤의 AdsPower 점유로 즉시 양보 → 최대 3회×120초 재확인 후 진행으로 수정(`dff82e3`), 10:41 재시작 반영. 이후 11:09~22:55 **8회 시도 8회 성공**, 경합 양보 0. 기록 `918269b`(VALIDATION `stepG_friend_automation_daily_run_260905` + ERR-121).

**2) 260906 — 8건, 장애 0:** missed·절전·경합·checkpoint 전부 0인데 8건. 발송 간격 105~121분(평균 113.6분). 17:56·21:52 회차는 PG-023 소진 후 PG-018→PG-011→PG-001까지 자동 그룹 순회.

**3) 260907 — 원인 확정 + 간격 조정 (ERR-139):** 처리량을 설정 간격(90분)으로 산정한 오류(실측 평균 113.6분 → 15시간에 8회가 상한). 실측 비율로 역산해 `.env` `INTERVAL_MIN=65`/`JITTER_SEC=1200`(회장 직접, 11:18 재시작, 배너·`Added job` 확인). **실발송 10건, 21:49:52 일일 상한 최초 도달.** 13:40~16:21 checkpoint 로그는 pytest 노이즈로 판정(ERR-139 근거). 같은 날 회장 요청으로 Instagram Like 자동화용 클릭 규칙 10줄 요약을 제공(파일 변경 없음 — Like 구현 자체는 아래 `260907 — Instagram Like` 항목).

**사후 확인(260917 로그):** 09-08·09-09·09-10 연속 10건. 09-11(0)·09-12(4)·09-14(3)·09-16(3) 미달일은 이 항목 범위 밖, 원인 미조사.

**상태변경 총계:** 코드 2파일(`launcher/main.py`·`tests/test_launcher_friend_job.py`, `dff82e3`). 시스템 전원설정(회장 직접, 260905). `.env` 2줄 값 변경(회장 직접, 260907). 실제 Meta 친구요청은 scheduler 무인 발송(260905 8 · 260906 8 · 260907 10). Airtable Write 0.

**Rollback:** 간격은 `.env` 두 값을 90/1800으로 되돌리고 재시작. 경합 재시도는 `git revert dff82e3`.

commit: `dff82e3`(fix) · `918269b`(docs) — push 완료(260917 origin/master 포함 확인). 이 백필 기록(ERR-139 · VALIDATION 행 · 이 항목): 회장 승인(260917)으로 docs 단일 커밋에 포함
push: 회장 승인(260917) — 같은 날 push
