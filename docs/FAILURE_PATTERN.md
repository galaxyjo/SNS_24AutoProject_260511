...
## FP-082 | 대화형 셸에서 정상 동작하는 외부 명령(git 등)이 실제 서비스 계정 환경에서도 동일하게 동작한다고 가정하면 안 된다

**설명:** Windows 서비스(NSSM 등)로 상시 구동되는 프로세스는 사람이 로그인해 여는 대화형 셸과 PATH·권한·작업 디렉터리가 다를 수 있다. Claude Code 세션의 Bash/PowerShell에서 어떤 외부 명령(`git`, 그 외 CLI 도구)이 정상 동작하는 것을 확인했다고 해서, 그 명령을 호출하는 코드가 실제 24시간 서비스 프로세스 안에서도 똑같이 동작한다고 가정하면 안 된다 — 두 실행 환경은 서로 다른 컨텍스트다. `subprocess`로 외부 실행파일을 호출하는 코드는 그 실행파일이 없거나 PATH에 없는 환경에서 예외를 던지는데, Fail-open 설계(예외를 조용히 삼키고 빈 값으로 넘어감)와 결합되면 실패 자체가 로그에 드러나지 않아 원인 특정이 더 어려워진다.

**증상:** 260811 — `health_monitor.record_boot_commit()`이 대화형 세션에서는 `git rev-parse HEAD`가 정상 동작해 문제없이 검증됐으나, 실제 NSSM 서비스 계정으로 기동된 `launcher/main.py` 프로세스 안에서는 같은 명령이 조용히 실패해(Fail-open) `commit=""`으로 기록됨 — 정작 필요한 순간(재시작 반영 검증)에 관측 기능 자체가 무력화돼 있었다. 상세는 ERR-111 참조.

**예방:** (1) 상시 구동되는 서비스 프로세스 안에서 실행될 코드가 외부 실행파일(`subprocess`)에 의존한다면, 가능하면 그 의존성을 제거하고 언어 표준 라이브러리나 파일시스템 직접 읽기로 대체한다(예: `git rev-parse HEAD` 대신 `.git/HEAD`+`refs` 직접 읽기 — ERR-111 Fix 참조). (2) 새로 만든 관측·진단 기능은 대화형 세션에서의 테스트만으로 "검증 완료"로 선언하지 않는다 — 실제 그 기능이 동작해야 하는 대상 프로세스(서비스 계정 등)에서 최소 1회 재현 검증한다. (3) Fail-open 설계를 쓸 때는 실패 자체를 관측 가능하게 만드는 것을 함께 고려한다(예: 실패 원인을 로그에 남기는 것과 서비스 기동을 막지 않는 것은 상충하지 않는다).

**관련:** ERR-111, ERR-109

---

## FP-084 | 생성형 모델에게 "최대 N자"로 지시해도 항상 정확히 지키지는 않는다 — 계약 상한은 실측 분포로 정하고, 재시도 없는 즉시거부 설계에는 렌더링 쪽에도 안전장치를 이중으로 둔다

**설명:** 프롬프트에 "at most N characters"라고 명시해도 LLM은 그 지시를 대략적으로만 따른다(정확한 토크나이저 수준 카운팅을 하지 않음, 이 프로젝트가 통제할 수 없는 모델 자체의 한계). 이 상한을 "재시도 없이 즉시 거부"하는 Fail-closed 계약과 결합하면, 상한이 실제 출력 분포보다 조금만 타이트해도 거부율이 그대로 기능 전체의 실패율이 된다. 상한을 설계 시점에 "이 정도면 충분하겠지"로 직관적으로 정하지 않고, 실제 모델을 여러 번 호출해 관측된 길이 분포에 여유를 두고 정해야 한다.

**증상:** 260814 — `hero_card_content_builder.py`의 문자수 상한이 실제 Gemini 출력보다 타이트해(설계 시 직관으로 정함, 실측 없었음) aijomoojin 자동 슬롯이 2일간(260812~260813) 7연속 전패. 실측 6회 호출 결과 3/6이 상한을 1~5자 초과로 거부됨. 상세는 ERR-113 참조.

**예방:** (1) 렌더링/표시 계층에 폰트 자동축소 같은 구조적 안전장치가 이미 있다면(즉 상한을 넘어도 최종 결과물이 시각적으로 깨지지 않는다면), 생성 계약의 문자수 상한을 "실측 분포+15~30% 여유"로 타이트하게 잡지 말고 처음부터 "명백히 잘못된(문단급) 응답만 걸러내는 최후 방어선" 수준으로 넉넉하게 잡는다 — ERR-113에서 15~50% 여유로 1차 완화했음에도 실제 자동 슬롯에서 재발했고(08:00/11:00 슬롯), 결국 2배 이상 여유(예: 28→80)로 재조정하고서야 안정됐다. "실측값에 적당히 여유를 둔다"는 접근 자체가, 안전장치가 이미 하위 계층에 있는 상황에서는 여전히 과도하게 보수적일 수 있다. (2) 상한을 한 번 조정한 뒤 실측 재현 테스트(예: 6회)에서 개선을 확인했다고 해서 "해결됨"으로 선언하지 않는다 — 실제 자동 슬롯(운영 트래픽)에서 1~2회 더 재현되는지 반드시 후속 관찰한다(ERR-113은 실측 재현 5/6 확인 후 RESOLVED로 잠정 종결했으나, 몇 시간 뒤 실제 자동 슬롯에서 2/3이 다시 실패해 재개됨). (3) 길이 상한이 "재시도 없는 즉시 거부"와 결합될 때는 특히 위 (1)이 중요하다 — 재시도가 없으므로 상한 위반률이 그대로 기능 실패율이 된다. (4) 실패율이 예상보다 높게 관측되면, "모델이 이상하다"거나 "안전 문제가 있다"고 추정하기 전에 먼저 어느 stage에서 무슨 error_code로 실패하는지 로그로 직접 확인한다(ERR-113에서 `DAILY_IMAGE_CAP_EXCEEDED`를 실측 DB 조회로 배제하고 나서야 진짜 원인에 도달함).

**관련:** ERR-113, ERR-110

---

## FP-083 | "글자 1개라도 있으면 전체 차단"류 이진 필터는 정상 콘텐츠와 오염 콘텐츠를 구분하지 못한다 — 비율 기반으로 설계하고 임계치는 반드시 실측으로 정한다

**설명:** 특정 언어/패턴의 "존재 여부"만으로 콘텐츠 전체를 통과/차단시키는 이진(all-or-nothing) 필터는, 그 언어/패턴이 텍스트 전체를 지배하는 경우(의도한 차단 대상)와 정상 콘텐츠에 극소량만 섞인 경우(오탐)를 구분하지 못한다. 다국어 대상 판매 게시물처럼 정상 콘텐츠가 여러 언어를 함께 포함하는 경우 이 취약점이 특히 잘 드러난다. 비율 기반으로 바꾸는 것 자체는 상식적인 해법이지만, **임계치를 "적당히 그럴듯한 수치"(예: 다른 곳에 쓰던 0.2를 그대로 재사용)로 정하면 실제로는 아무것도 해결하지 못하거나 반대 방향 회귀를 만들 수 있다** — 반드시 (a) 차단하려는 원래 대상 샘플과 (b) 통과시키려는 새 대상 샘플 양쪽을 실측해 값이 실제로 갈리는 지점을 확인해야 한다.

**증상:** 260812 — `_has_excluded_language()`를 비율 기반으로 바꾸며 처음 20%를 제안(다른 함수 `_korean_ratio()`의 0.2 관례를 무비판적으로 재사용)했으나, 회귀테스트 작성 중 실제 베트남어 문장 4종을 측정하니 15~28% 범위로 20%와 겹침 — 그대로 채택했다면 ERR-033에서 이미 "정상 설계"로 확정된 차단 대상(전체가 베트남어인 게시글)을 오히려 통과시키는 반대 방향 회귀가 됐을 것. A003/A505 실제 오탐 사례는 실측 0.56%로, 5%면 양쪽 다 안전하게 구분됨을 확인 후 채택. 상세는 ERR-112 참조.

**예방:** (1) 비율/임계치 기반 필터를 설계·수정할 때는 "다른 곳에서 쓰던 값"이나 "그럴듯해 보이는 수치"를 그대로 재사용하지 않는다. (2) 회귀테스트에 "차단 유지해야 하는 원래 대상"과 "이제 통과시키려는 새 대상" 양쪽의 실제(또는 실제와 유사한) 샘플을 모두 포함시키고, 두 그룹의 실측값 사이에 충분한 마진이 있는지 코드로 확인한 뒤 임계치를 정한다. (3) "테스트가 통과했다"를 "임계치가 옳다"와 동일시하지 않는다 — 테스트 자체가 잘못된 가정(부적절한 임계치)으로 작성되면 통과해도 무의미하다.

**관련:** ERR-112, ERR-033, FP-022

---

## FP-085 | 검증(grounding-check) 대상이 모델에게 실제로 준 입력의 부분집합이면, 그 입력의 나머지 부분(title 등)이 정답이어도 날조로 오판정한다

**설명:** `_build_prompt(core_message, title)`처럼 프롬프트에 여러 필드를 근거로 함께 넣어놓고, 사후 검증(fabrication/grounding check)은 그중 한 필드(core_message)만 대조하면, 모델이 "다른 근거 필드(title)"에서 정확히 가져온 내용까지 "이 필드엔 없는 내용"으로 오판정한다. 특히 숫자·버전·연도처럼 title에 흔히 등장하는 값이 취약하다. 실패 처리 자체는 안전하게 동작(Fail-closed)해도, 그 topic이 "사용됨"으로 기록되지 않는 설계와 맞물리면 같은 topic이 계속 재선택되며 이후 슬롯까지 연쇄로 막히는 더 큰 증상으로 확대된다.

**증상:** 260817 — hero card Producer가 topic 3.6("NIST AI Risk Management Framework, AI RMF **1.0**")을 뽑을 때마다 3회 연속(05/08/11시) `POSSIBLE_FABRICATION`으로 실패. 실제 Gemini 응답을 직접 뜯어보니 헤드라인이 제목을 그대로 옮긴 "1.0"이 원인이었고, `carousel_content_builder.py`도 동일 패턴(prompt엔 title 포함, 검증은 core_message만)을 갖고 있어 아직 안 걸렸을 뿐 같은 취약점이 잠재해 있었다. 상세는 ERR-115 참조.

**예방:** (1) grounding-check을 설계할 때는 "검증 대상 = 실제로 모델에게 프롬프트로 준 근거 전체"인지 항상 확인한다 — 프롬프트 입력 필드를 늘리면 검증 대상도 같이 늘려야 한다. (2) 실패 시 재시도 없이 그 항목만 스킵하는 설계라면, 그 항목이 "왜 실패했는지" 재선택 로직이 회피할 수 있는지도 함께 설계 검토한다(하나의 topic 버그가 그날 전체를 막을 수 있는지).

**관련:** ERR-115, ERR-113

---

## FP-086 | "관리자 권한이 필요 없다"를 "승인이 필요 없다"로 착각하면, read-only 조사 중에 상태변경 명령을 그냥 실행해버릴 수 있다

**설명:** 이 프로젝트에서 실제로 승인 없이 실행하면 안 되는 관리자 권한 명령(서비스 재시작 등)은 이 세션 자체가 admin 권한이 없어 물리적으로 실행이 막혀 있어, "명령을 내가 직접 못 침"이라는 사실이 곧 "승인 게이트"처럼 작동해왔다. 그런데 `powercfg /setdcvalueindex` 같은 시스템 설정 변경 명령 중 일부는 admin 권한 없이도 그냥 실행된다 — 이 경우 "권한 상 막히지 않았다"는 사실을 "이건 read-only 진단 명령이다"와 혼동하면, AUTONOMOUS INVESTIGATION MODE(read-only 조사는 매번 승인 없이 연속 실행 가능)의 적용 대상이 아닌 상태변경 명령을 그 모드 안에서 그냥 실행해버리게 된다. 권한 요구 여부와 승인 필요 여부는 서로 다른 축이다.

**증상:** 260824 — `powercfg /query`로 DC(배터리) STANDBYIDLE 값을 확인하는 read-only 조사 도중, 검증 목적으로 `powercfg /setdcvalueindex ... 0`을 실행 — admin 권한이 필요 없어 그냥 성공해버렸고, 실행 후에야 이것이 "시스템 설정 변경"(CLAUDE.md AUTONOMOUS INVESTIGATION MODE 명시적 제외 대상)이었음을 인지해 회장에게 자진신고. 상세는 ERR-116 참조.

**예방:** (1) 어떤 명령이 상태를 바꾸는지 여부는 "지금 이 세션에서 실행이 되는가/안 되는가"가 아니라 "이 명령의 본질이 조회인가 변경인가"로 판단한다(`Get-*`/`query`/`status` 계열 vs `Set-*`/`setacvalueindex`/`setdcvalueindex`/`New-*`/`Remove-*` 계열처럼 동사 자체로 먼저 분류). (2) 새로운 도구·명령을 처음 쓸 때는 "이게 실행됐다"보다 "이게 뭘 바꿨는가"를 스스로 먼저 묻는다. (3) CLAUDE.md AUTONOMOUS INVESTIGATION MODE의 제외 목록은 일반 문구("시스템 설정 변경")보다 실제로 쓰는 구체적 명령 계열(`powercfg /set*`, 레지스트리 편집 등)을 예시로 명시해야 이런 착각이 줄어든다.

**관련:** ERR-116

---

## FP-087 | "DOM에서 요소를 찾았다"를 "그 요소를 클릭할 수 있다"로 착각하면, 클릭은 되지만 아무 일도 일어나지 않는다

**설명:** Selenium으로 요소를 찾는 것(`find_elements` + aria-label 매칭)과 그 요소가 화면에 실제로 렌더돼 클릭 가능한 상태인 것은 별개다. 특히 Facebook처럼 접근성 노드를 미리 만들어두고 실제 버튼은 일부만 레이아웃하는 UI에서는, aria-label만 보고 잡은 요소가 `rect 0x0`인 경우가 흔하다. 이때 `arguments[0].click()`(JS)은 예외 없이 "성공"하지만 프레임워크 핸들러는 동작하지 않아 **아무 부작용 없이 조용히 실패**한다. 로그에는 "클릭했다"만 남아 실패 원인을 오래 못 찾는다. 반대로 실렌더 버튼인데 Selenium native click이 obstruction을 과엄격하게 판정해 `ElementClickInterceptedException`으로 거부하는 경우도 있어, "찾음 / 보임 / 클릭 가능 / 어떤 클릭 방식이 먹히는가"는 각각 따로 확인해야 한다.

**증상:** 260903 — 회원목록에서 `친구 {이름}님 추가` 버튼 4개를 정상 추출했으나 그중 1개만 `rect>0`이었고, 나머지 3개(0x0)에 JS 클릭 6회 → 실제 친구요청 발송 0건(대상 프로필 재확인으로 확정). 이어서 프로필의 실렌더 버튼에는 native click이 `ElementClickIntercepted`로 거부됐고, 260902에 이미 발송을 실증한 JS click으로 바꾸자 성공. 상세는 ERR-118 / ERR-119.

**예방:** (1) 클릭 전 게이트를 코드로 고정한다 — `is_displayed()` **그리고** `rect.width>0 and rect.height>0`. 둘 중 하나만으로는 부족하다. (2) 클릭 후에는 반드시 **대상 본인의 상태 전이**를 확인한다(요청됨/요청 취소). "클릭 예외가 없었다"를 성공으로 삼지 않는다. (3) 클릭 방식(native/JS)은 화면별로 실증된 것을 쓴다 — 한 방식으로 통일하려다 이미 검증된 경로를 잃지 않는다. (4) 실패 시 "발송됐는데 감지를 못한 것"과 "애초에 발송이 안 된 것"을 구분하기 위해, 대상 프로필을 read-only로 다시 열어 상태를 확인한다.

**관련:** ERR-118, ERR-119, INC-055

---

## FP-088 | 화면 한 장/표본 하나로 모집단을 조기 결론내면, 실제로는 충분한 대상을 "없다"고 잘라버린다

**설명:** 목록형 UI에서 현재 화면에 보이는 개수를 그대로 "이 그룹의 전체 후보 수"로 결론내면, 스크롤·다음 페이지·다른 그룹에 있는 대상을 통째로 놓친다. 특히 실패가 반복되는 상황에서는 "대상 자체가 부족하다"는 설명이 그럴듯하게 들려 조기 결론으로 굳어지기 쉽고, 그 결론이 이후 설계(스케줄 간격, 목표 건수, 확장 여부)까지 잘못 이끈다.

**증상:** 260903 — 테스트용 Ground Truth 그룹에서 친구가능 회원이 4명(그중 비한국 2명)뿐인 것을 확인하고 **"이 그룹 후보는 4명뿐 → 하루 10건은 여러 그룹이 필요하다"**고 보고했다. 회장이 "한 화면에서 4명 보였다고 이 그룹 후보가 4명뿐이라고 결론내지 마라"(Human SOP §2)로 지적. 실제로 Airtable의 실 운영 그룹(PG-023 `NHÓM NGUỒN SỈ MỸ PHẨM CHÍNH HÃNG`)을 열자 **친구가능 8명**이 즉시 잡혔고, 하루 10건은 무리 없이 달성 가능한 규모였다. 조기 결론이 "후보 부족"이라는 잘못된 제약을 만들어냈다.

**예방:** (1) "N개뿐"이라는 결론은 **실제로 끝까지 처리해 소진을 확인한 뒤**에만 낸다 — 스크롤·다음 그룹까지 돌려보기 전에는 UNKNOWN으로 남긴다. (2) 테스트용 표본(Ground Truth)과 실제 운영 모집단(Airtable 운영 그룹)을 구분한다 — 테스트 표본의 특성을 운영 전체로 일반화하지 않는다. (3) 사람 직원이 하는 업무를 자동화할 때는 "몇 명인지 분석"이 아니라 "사람처럼 계속 내려가며 처리"가 기본 동작이어야 한다(Human SOP 핵심 원칙).

**관련:** ERR-118, INC-055

## FP-089 | 감시 경로가 조용히 끊기면 "알림 0건"이 정상처럼 보인다

**설명:** 장애 알림은 "보낼 일이 없어서 0건"과 "보내는 경로가 끊겨서 0건"이 겉으로 구분되지 않는다. 게다가 성공 발송을 로그로 남기지 않는 알림 모듈에서는 로그에 발송 흔적이 없다는 사실만으로 "알림이 안 갔다"고 오판하기도 쉽다.

**증상:** 260913 — AdsPower 38시간 다운(ERR-122) 동안 크롤링 전체실패 78회에도 크롤링 경보 0건. 원인은 7/30 함수 삽입으로 `@handle_errors`가 다른 함수에 붙은 것(ERR-123). 같은 조사에서 `slack_notifier`가 성공 시 로그를 남기지 않는 구조를 확인하지 않고 "Slack 알림 0건"이라 보고했다가, 친구요청 잡은 20건을 발송했다고 정정함.

**예방:** (1) 외부 의존 프로세스는 잡 실패와 별도로 전용 생존 감시를 둔다(DOWN/복구 전이 1회씩). (2) 잡 함수의 오류 경보 부착 여부를 테스트로 고정한다. (3) "알림 0건" 판단 전에 알림 모듈이 성공을 기록하는지부터 확인한다.

**관련:** ERR-122, ERR-123

## FP-090 | 외부 서비스·엔진 실패를 도메인 판정으로 조용히 바꾸는 코드

**설명:** 번역·OCR 같은 보조 서비스가 실패했을 때 `except: return ""` 또는 `return True`로 처리하면, 인프라 장애가 "무관 게시물(제외)" 또는 "워터마크 없음(통과)"이라는 비즈니스 판정으로 위장된다. 로그는 정상 판정처럼 보이고, 결과는 수집 0 또는 무검사 게시가 된다.

**증상:** 260914 — Google 번역 `TooManyRequests` → `""` → 관련 게시물까지 제외(9/12~14 99.9%, ERR-125). OCR 엔진 import 실패 → 통과 처리 1,754건, 차단 0건(ERR-126).

**예방:** (1) 보조 서비스 실패는 별도 상태(ERROR)로 분리하고 로그 레벨을 올린다. (2) 안전 필터는 Fail-closed, 수집용 판정은 실패 시 대체 경로(원문 키워드 선검사)를 둔다. (3) 판정 로그와 실패 로그의 문구를 다르게 한다.

**관련:** ERR-125, ERR-126

## FP-091 | Mock 테스트는 경로·환경·셸 이스케이프 결함을 잡지 못한다

**설명:** 외부 엔진을 Mock하면 설정값(실행파일 경로 등)이 실제로 쓰이지 않으므로, 그 값이 깨져도 테스트는 통과한다. 같은 편집 경로로 작성한 테스트는 깨진 값을 그대로 "기대값"으로 기록해 결함을 오히려 고정할 수 있다.

**증상:** 260914 — 셸 heredoc이 백슬래시를 줄여 `tesseract.exe` 경로에 TAB이 들어감(ERR-127). Mock 기반 OCR 테스트 전원 통과. 같은 방식으로 추가한 경로 테스트도 TAB 값을 기대하도록 기록됨. 재시작했다면 모든 이미지 게시물이 OCR_ERROR로 차단될 상황.

**예방:** (1) Mock 테스트와 별개로 실엔진 스모크(합성 입력)와 설정값 바이트 검사를 둔다. (2) 백슬래시가 들어가는 코드는 셸 heredoc으로 작성하지 않는다. (3) 경로 상수는 raw 문자열로 기대값을 단언한다.

**관련:** ERR-127

## FP-092 | 안전장치 계약이 후속 변경에서 다른 책임과 섞여 조용히 의미가 바뀐다

**설명:** "보류"용 안전장치가 나중에 추가된 "거절" 처리 함수를 재사용하면, 특정 설정 조합에서만 의미가 바뀌어 원 커밋의 주석·설명과 실제 동작이 어긋난다. 테스트가 그 조합을 검증하지 않으면 발견되지 않고, 이후 설계자는 원 커밋 설명을 믿고 잘못된 전제로 작업한다.

**증상:** 260915 — Kill Switch(7/30, "ready 유지")가 7/31 Identity Gate 도입으로 `_identity_reject()`를 쓰게 되어 `PUBLISH_TEXT_GATE_ENABLED=true`에서 `rejected`로 기록(ERR-129). Canary 설계 시 7/30 커밋 설명만 확인해 "보류된다"고 안내, 실제 게시물 2건 거절.

**예방:** (1) 안전장치를 Canary에 쓰기 전에 현재 코드의 호출 경로를 설정 조합별로 확인한다(커밋 설명 신뢰 금지). (2) 보류와 거절은 다른 함수·다른 로그로 분리하고 각각 테스트로 고정한다. (3) 안전장치의 부수효과(친구요청 동시 정지 등)도 호출부 검색으로 확인한다.

**관련:** ERR-129

## FP-093 | 화면에 보이지 않는 브라우저로 동적 페이지를 수집하면 첫 화면만 렌더링된다

**설명:** 무한 스크롤·가상화 피드는 뷰포트 가시성을 기준으로 내용을 채우고 다음 항목을 불러온다. 창이 최소화·숨김 상태면 스크롤 명령은 동작해도 추가 로드가 일어나지 않아, 수집기는 "게시물이 원래 적다"거나 "스크롤 로직 문제"로 오판하기 쉽다.

**증상:** 260914~260915 — 크롤러 `posts=3`(실제 글 1 + 빈 틀 2) 고정. 스크롤 확대·최신순·가입 여부 가설은 실측으로 모두 기각. 계측에서 `visibilityState=hidden`, 창 0×0 확인, A/B에서 포커스 흉내만으로 1→32개(ERR-128).

**예방:** (1) 브라우저 수집 결과가 이상하면 스크롤 로직보다 먼저 `visibilityState`·창 크기·`scrollHeight` 변화를 계측한다. (2) 가설은 조건 하나만 바꾼 A/B로 확정한다.

**관련:** ERR-128

## FP-094 | 테스트가 운영 로그·상태파일에 기록해 사고 조사를 오염시킨다

**설명:** 중앙 로거와 상태파일이 테스트에서 격리되지 않으면, 테스트가 만든 가짜 오류·가짜 식별자가 운영 로그에 섞인다. 이후 영향범위 조사에서 실제 사고와 테스트 흔적을 매번 구분해야 하고, 잘못 합산할 위험이 생긴다.

**증상:** 260914 — 가짜 `disk full` 오류 8건, 가짜 `IDENTITY_REJECTED | rid=rid1/rec4/rec5`, boot state 덮어쓰기(ERR-131). 테스트가 실제 Slack 경보까지 발송(ERR-124).

**예방:** (1) 신규 테스트는 로거·Slack·상태파일 경로를 파일 단위 autouse로 격리한다. (2) 운영 로그 조사 시 테스트 실행 시각과 가짜 식별자 패턴을 먼저 제외한다. (3) 로거 격리 자체는 별도 과제로 해결한다.

**관련:** ERR-124, ERR-131

## FP-095 | 운영이 실제로 읽는 설정 출처를 확인하지 않고 수정안을 낸다

**설명:** 같은 값이 파일(예: `configs/accounts.json`)과 외부 저장소(Airtable) 두 곳에 있고 Feature Flag로 출처가 갈리면, 눈에 보이는 파일만 고쳐도 운영에는 반영되지 않는다. 캐시까지 있으면 "고쳤는데 그대로"가 재시작 전까지 드러나지 않는다.

**증상:** 260916 STEP 5 — 실패 그룹 제외안을 `accounts.json` 1줄 제거로 승인받았으나 `.env` `CRAWL_TARGET_SOURCE=airtable`이라 launcher가 크롤 URL을 Airtable `Crawl_Targets`로 덮어쓰고 있었음(ERR-133). 수정 직전 확인으로 무효 변경은 막음.

**예방:** (1) 설정·대상 목록 수정안 전에 로더 코드의 분기 Flag와 실제 로드 로그("… 방식 적용 | N건")를 먼저 확인한다. (2) 캐시 여부(재시작 필요성)를 수정안에 함께 적는다.

**관련:** ERR-133

## FP-096 | 숨김 콘솔 예약작업에서 GUI(Electron) 앱을 띄우면 작업 인스턴스가 끝나지 않는다

**설명:** 작업 스케줄러가 `conhost --headless`로 헬퍼를 실행하고 헬퍼가 Electron 앱을 `Start-Process`로 띄우면, Electron이 부모 콘솔에 붙어 conhost가 앱 생존 동안 남는다. 작업은 "실행 중"으로 유지돼 반복 점검이 IgnoreNew로 건너뛰어지고, 실행시간 한도에서 강제 종료된다.

**증상:** 260915 ERR-130 START Canary — START 후 10분 미종료, 다음 점검 2회 id=322, id=329 시간한도 종료(ERR-132). `ELECTRON_NO_ATTACH_CONSOLE=1` 적용 후 14초 정상 종료.

**예방:** (1) 예약작업·서비스에서 Electron 앱을 띄울 때 `ELECTRON_NO_ATTACH_CONSOLE=1`을 준다. (2) Canary 성공기준에 "작업 인스턴스 종료 시간·return code·다음 주기 정상 실행"을 포함한다(Mock 테스트로는 못 잡음).

**관련:** ERR-130, ERR-132

## FP-097 | 검증 하네스 자체의 PowerShell 함정이 판정을 오염시킨다

**설명:** Windows PowerShell 5.1에서 (1) 함수가 반환한 단일 CimInstance는 `.Count`가 비어 있고, (2) `@("-c", "문자열" + $변수)`는 콤마가 `+`보다 먼저 묶여 인자가 쪼개지며, (3) Electron 앱은 메인 1개 외에 `--type=` 없는 하위 프로세스(main/rpa 스크립트)를 가진다. 하네스가 이를 모르고 짜이면 정상 상태를 FAIL/ABORT로, 또는 무효 결과를 PASS처럼 보이게 만든다.

**증상:** 260915~16 — AutoRecover Canary 사전점검이 루트 프로세스 수를 4 또는 빈값으로 읽어 2회 ABORT(AdsPower 무접촉), PYTHONPATH 검증 S3~S5가 인자 쪼개짐으로 빈 출력(무효) → 괄호 수정 후 재실행.

**예방:** (1) 함수 결과는 항상 `@(...)`로 감싸 센다. (2) 문자열 결합 인자는 괄호로 묶는다. (3) 프로세스 중복 판정은 "부모가 같은 이름이 아닌 루트 프로세스" 기준으로 한다. (4) 하네스 결과가 비어 있으면 PASS로 세지 않고 무효로 표시한다.

**관련:** ERR-132

---

## FP-098 | 기존 액션의 안전 계약을 새 액션에 복사하면서 액션 의미 차이를 검토하지 않는다

**설명:** 검증된 형제 함수(`follow_once`)를 REUSE해 새 액션(`like_once`)을 만들 때, dedup·한도·기록 규칙까지 그대로 복사한다. 그러나 액션마다 "실패 후 재시도가 안전한가", "attempt와 성공 상태를 분리해야 하는가"가 다르다.

**증상:** 260907 — Like 기술 실패 1건이 로컬·SSOT dedup에 남아 같은 게시물 재시도를 영구 차단(ERR-138). Audit 삭제 금지 조건과 충돌해 수정 후 재검증이 막혔다.

**예방:** 계약을 복사할 때 규칙별로 "이 액션에서도 같은 의미인가"를 한 줄씩 적는다 — 특히 (1) 영구 차단 대상이 성공만인지 시도 전체인지 (2) 실패 기록과 재시도 허용 여부 (3) 한도가 attempt 기준인지 성공 기준인지. 공용 헬퍼를 바꾸지 말고 새 액션 전용 경로로 분리한다(기존 액션 회귀 0).

**관련:** ERR-138

---

## FP-099 | 신규 브라우저 프로필의 실행 전제를 확인하지 않고 Live 액션부터 실행한다

**설명:** 새 계정·새 AdsPower 프로필을 연결할 때 (1) 커널 버전과 chromedriver 일치 (2) 대상 플랫폼 로그인 세션 존재를 먼저 확인하지 않고 바로 Live 액션을 돌린다. 실패가 액션 로직 결함처럼 보여 원인 판정이 늦어지고, 실패 attempt가 Audit·일일 한도를 소모한다.

**증상:** 260907 `k1goch3g` — 커널 150 미설치 → 설치 후 chromedriver 144 불일치(ERR-136) → 정렬 후 로그아웃 상태로 클릭해 `state_not_confirmed`(ERR-137). Like 로직은 다른 계정에서 이미 성공한 상태였다.

**예방:** 새 프로필은 Live 전에 read-only 점검 2개를 통과시킨다 — (a) `browser/start` 응답의 `webdriver` 경로 커널 = 프로젝트 chromedriver 커널, attach 성공 (b) 대상 사이트 로그인 쿠키(Instagram: `sessionid`·`ds_user_id`) 존재. 둘 중 하나라도 아니면 액션을 실행하지 않는다. 프로필 설정 정렬은 UI가 아니라 AdsPower Local API(`user/update`)로 한다.

**관련:** ERR-136, ERR-137

---

## FP-100 | DOM에서 찾은 요소를 넓은 조상까지 올려 native click한다

**설명:** 아이콘을 찾은 뒤 클릭 대상을 넓은 조상 요소로 잡고 Selenium native `.click()`을 쓴다. native click은 요소 중심점을 hit-test하므로, 아이콘이 보이는데도 조상 중심이 다른 요소에 가려 intercepted가 난다. 반대로 DOM에 있어도 0x0 유령노드면 클릭이 아무 효과가 없다.

**증상:** 260907 Instagram Like 1차 `ElementClickInterceptedException`(ERR-135). 회장이 제시한 친구추가 성공 사례 클릭규칙(260903~)과 같은 계열 문제.

**예방:** (1) 가장 가까운 semantic 조상(`closest('button')` → `closest('[role=button]')`)만 사용 (2) 클릭 전 `is_displayed()` + rect>0 + 아이콘 포함 확인 (3) JS `arguments[0].click()` 1회, 재시도·좌표·ActionChains 금지 (4) 판정은 대상 영역(article 등) 안에서만 (5) 클릭 성공이 아니라 상태 전이로 성공 판정.
