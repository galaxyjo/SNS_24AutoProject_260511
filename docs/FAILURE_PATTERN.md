...
**예방:** (1) 생성형 모델에게 길이 제약을 지시하는 계약을 설계할 때는, 코드를 작성하기 전에 실제 모델을 여러 번(최소 5~6회, 서로 다른 입력으로) 호출해 관측된 길이 분포를 먼저 확인하고, 그 분포에 15~30% 여유를 둔 값을 상한으로 정한다 — 프롬프트에 적은 숫자를 그대로 코드의 하드 상한으로 복사하지 않는다. (2) 길이 상한이 "재시도 없는 즉시 거부"와 결합될 때는, 그 상한을 넘어도 최종 결과물이 시각적으로 깨지지 않도록 렌더링/표시 계층에 별도의 구조적 안전장치(예: 폰트 자동축소)를 이중으로 둔다 — 그러면 생성 계약의 상한을 다소 넉넉하게 잡아도 안전하다(ERR-113의 `_hero_fit_block_line()` 사례). (3) 실패율이 예상보다 높게 관측되면, "모델이 이상하다"거나 "안전 문제가 있다"고 추정하기 전에 먼저 어느 stage에서 무슨 error_code로 실패하는지 로그로 직접 확인한다(ERR-113에서 `DAILY_IMAGE_CAP_EXCEEDED`를 실측 DB 조회로 배제하고 나서야 진짜 원인에 도달함).
