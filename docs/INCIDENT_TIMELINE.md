...
**진행 상황:** 근본원인 가설 수립 완료(ERR-065) — `Start-N8n()`의 npx 대화형 설치 프롬프트가 non-interactive 컨텍스트에서 무기한 대기(hang)하는 것으로 추정, LocalSystem 계정 전환(ERR-058과 동일 클래스)이 악화 요인으로 의심. 확정을 위해서는 좀비 프로세스 강제종료 후 재현 테스트가 필요하나, 이번 조사는 회장 지시로 read-only 기록에 국한.

**해결:** 미해결. 회장 방침(260715): 안정화 작업을 먼저 완료한 뒤 n8n 진행 예정이며, 기존 n8n 설계(WF-01~05)도 재검토가 필요할 것으로 판단 — 이번엔 코드/프로세스 변경 없이 현황만 기록. 좀비 프로세스 종료나 watchdog.ps1 n8n 감시 블록 비활성화 등 실제 조치는 별도 승인 필요.
