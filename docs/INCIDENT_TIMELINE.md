...
## INC-048 | 학습 검토 그리드가 판단불가 상태의 죽은 이미지 55건에 막혀 신규 리뷰가 사실상 정지 (RESOLVED — 데이터 삭제로 해소, 260806)

**발생:** 2026-07-30 01:32~01:54 ICT 수집된 55건이 Facebook CDN 서명 만료로 이미지 접근 불가 상태가 됐고(정확한 만료 시점은 UNKNOWN — FB CDN 서명이 통상 며칠 내 만료되는 특성상 260730~260805 사이로 추정), 회장이 260806 리뷰 그리드에서 이를 발견할 때까지 PENDING 상태로 큐 맨 앞에 남아있었다.

**영향:** 리뷰 그리드(`fetch_pending_candidates`)가 `collected_at` 오름차순으로 후보를 보여주는 구조라, 이 55건이 항상 배치 맨 앞을 차지해 그 이후 수집된(이미지 정상) 후보의 실질적 리뷰를 방해했을 가능성이 있다 — 다만 이 기간 동안 다른 배치가 정확히 얼마나 처리됐는지는 별도로 추적하지 않아 UNKNOWN.

**진행 상황:** 260806 read-only 조사로 원인 확정(ERR-106) — 이미지 재호스팅 코드(260805, 커밋 `cba1cc2`) 자체는 정상 동작 중이었고, 그 수정 이전 수집분만 문제였음.

**해결:** 백필(재업로드) 시도 → 원본 자체가 서버측 재요청에도 403(복구불가 확정) → 회장 승인 하 55건 전부 Airtable에서 삭제, 재조회로 PENDING 0건 확인. 이후 신규 크롤로 수집된 후보가 정상 이미지로 표시됨을 회장이 스크린샷으로 직접 확인(실사용 재검증 완료).
...
## INC-051 | Live 스케줄러 프로세스가 ERR-107 수정 커밋 이후에도 재시작 없이 구코드로 계속 동작, 260810 17:00:33 정규 슬롯에서 동일 증상 4번째 재현 (RESOLVED — 관측 기능 추가, 260811)

**발생:** 260810 세션에서 ERR-107 Phase A.5 수정을 커밋하고 별도 스크립트 실행으로 "production_verified"라 보고했으나, 이는 Live 스케줄러 프로세스(`launcher/main.py`, 05:56:43 기동)가 아닌 단독 실행 검증이었다. 그 프로세스는 그날 20:02:54까지 재시작되지 않아 하루 종일 수정 전 구코드로 계속 동작.

**영향:** 17:00:33 정규 슬롯에서 ERR-107과 동일한 HTTP 400 OUTCOME_UNKNOWN이 4번째로 재현됨 — 수정이 "완료"됐다고 보고된 이후에도 실제 운영에서는 여전히 미해결 상태였던 것과 동일한 결과.

**발견 경위:** 260811 08:36 ICT "오늘오전 자동 안올라갔어" 회장 보고를 계기로 watchdog.log 기동 타임스탬프를 커밋 타임스탬프와 대조하며 발견.

**해결:** `health_monitor.py`에 코드-신선도 관측 기능(`record_boot_commit()`/`get_code_freshness_status()`) 추가(ERR-109/FP-080 참조) 후, 관리자 PowerShell로 프로세스 재시작 → 이후 정규 슬롯에서 신규 컨테이너 기준 Phase A→A.5→B 전 구간이 실제로 실행돼 성공(`ig_media_id=18090013187127904`, `17954648910230969`)함을 확인.

**재발 방지:** ERR-109 Fix 참조. **한계 명시** — 이 관측 기능은 사람이 확인 후 수동 재시작하는 것을 전제로 하며, 코드 변경 감지 시 자동 재기동까지는 이번 범위에 포함되지 않는다.

**관련:** ERR-107, ERR-109, FP-078, FP-080, INC-050

---

## INC-052 | yuna18253 FB 크롤 타겟 4개 중 2개(A003/A005)가 3.5일간(최소 260808~260811) 실질 콘텐츠 0건 생산 — 비율기반 필터 전환으로 해소 (RESOLVED, 260812)

**발생:** Active FB 크롤 타겟 A002/A003/A004/A005 중 A003(FB_뷰티도매2)·A005(FB_신규2) 두 곳이 로그 보존 구간 전체(2026-08-08 13:18~2026-08-11 23:27, 189+ 사이클/그룹)에서 Instagram_Posts 저장 0건. 두 그룹의 매 사이클 유일한 실질 게시물(같은 작성자의 다국어 도매 템플릿)이 매번 `_has_excluded_language()`에 걸려 번역·키워드 필터 진입 자체가 불가능했음.

**영향:** 4개 Active 타겟 중 2개(50%)가 사실상 개점휴업 상태로 최소 3.5일 지속 — 그 기간 전체 FB 크롤 생산량 34건 중 A002(29건)·A004(5건)에만 편중, A003/A005 기여 0건. 계정 정지·오게시·데이터유실 등 급성 장애는 아니었으나 콘텐츠 파이프라인 처리량 저하가 조용히 누적된 케이스.

**발견 경위:** 회장 지시("yuna18253 계정 크롤링 다시 점검, 페이스북 크롤링 된게 많이 안보인다")로 260812 세션에서 Airtable Instagram_Posts를 계정·소스그룹별로 전수 대조하며 발견.

**해결:** ERR-112 Fix — `_has_excluded_language()`를 이진(글자 존재 여부) 판정에서 비율(5%) 기반으로 전환. 회장 승인 하 `Restart-Service SNS_Watchdog` 재시작으로 라이브 검증, 같은 날 다음 사이클에서 A003·A005 둘 다 첫 저장 확인(`A-F4-260812-001`/`A-F6-260812-001`).
...
## INC-055 | STEP 3-G YUNA 친구요청 자동화 — 260902~260904 3일간 실질 발송 정체(총 3건) 후 24시간 분산 스케줄로 정상화

**기간:** 2026-09-02 ~ 2026-09-04 (KST)

**운영 영향:** 친구요청 자동화(퍼널 1단계 Prospect→Friend)가 3일간 실질적으로 진행되지 못했다. 260902 2건(그중 1건은 한국인이라 회장이 수동 취소), 260903 3건. 다른 파이프라인(Instagram 게시·DM·댓글·크롤)에는 영향 없음 — 친구 자동화는 scheduler OFF 상태로 격리돼 있었고, Meta 차단·CAPTCHA·계정 제한은 전 기간 0건.

**타임라인:**
- **09-02**: STEP 3-G+R(이름 국적판정 폐기 → location Evidence 전용) 적용. 라이브 검증에서 8/8 전원 `korea_evidence` 오판 발견 → **ERR-117**(그룹명 오염). S1 수정으로 오염 0 확인.
- **09-03 오전**: 회장이 타겟 정의 정정("해외 B2B 도매상이 바이어, 중국공장은 스팸"). whitelist(해외 명시만) → **blacklist(한국인·중국공장만 제외)** 전환 승인. S3 구현.
- **09-03 10:29**: 첫 Live Canary 실패 — 목록 버튼 `rect 0x0` 유령노드에 JS 클릭 6회, 실발송 0 → **ERR-118**. read-only로 발송 0 확정(부작용 없음).
- **09-03 16:07~16:20**: 목록 클릭 가능성을 probe 8회로 최종 확인 — 페이지 전체 1822px, 끝까지 스크롤해도 렌더되는 친구버튼은 1개뿐 → 목록 클릭 경로 CLOSED, 프로필 fallback 확정.
- **09-03 16:51**: Live Canary 2차 실패 — 프로필 버튼에 native click이 `ElementClickIntercepted` → **ERR-119**. 발송 0.
- **09-03 17:05**: **프로필 JS click으로 수정 → Hnin Thu Zar 실제 발송 성공**, read-only 재확인으로 `요청 취소` 지속 확인(live path verified).
- **09-03 19:34~19:39**: Human SOP 코드(성공 후 계속·실패 skip·그룹 순회)로 PG-023에서 2건 추가 발송(Duc Trung Nguyen, Ly Ly). 세션 중단으로 조기 종료, 잔여 브라우저 19:41 정리.
- **09-03 21:32~22:19**: 회장 지시로 **24시간 분산 10회** 전환(랜덤 90~180분→후에 60~120분, 08~23시 제한, 실행당 1건, AdsPower 사용 중 양보). scheduler ON.
- **09-03 23:14**: 첫 자동 발사 — 활동시간대 밖(23시)으로 설계대로 skip.
- **09-04 06:27**: 간격 재조정(`INTERVAL_MIN=90`) 중 `SCHEDULE_ENABLED` 유실로 잡 미등록 → **ERR-120**. 3줄 복구 후 06:30:27 정상 등록 확인.

**최종 상태:** scheduler ON, 08~23시 60~120분 랜덤 간격, 실행당 1건, Airtable 26개 active 그룹 순회, 하루 상한 10건. 09-04부터 하루 10건 목표 실행 가능.

**관련:** ERR-117, ERR-118, ERR-119, ERR-120, FP-087, FP-088

## INC-056 | AdsPower Local API 38시간 다운 — 크롤링·친구요청 중단, 무경보

**기간:** 2026-09-11 02:33 ~ 2026-09-12 16:34 (KST)

**운영 영향:** Facebook 크롤링 78회 계정 전체실패(수집 0), YUNA 친구요청 9/11 0건·9/12 4건(평소 10건). AI 자동포스팅(aijomoojin)은 영향 없음(25/25). 장애를 알리는 경보 없음 — 회장이 9/13 정검 요청으로 발견.

**타임라인:**
- **09-11 02:29:18**: Windows Update 자동 재시작(`MoUsoCoreWorker.exe`), 02:31:27 2차 재시작(`TrustedInstaller.exe`), 02:32:30 로그온.
- **09-11 02:32**: launcher 재기동. AdsPower는 실행되지 않음(AdsPower 로그 9/11 기록 0건).
- **09-11 02:33:59**: 첫 `WinError 10061` 크롤링 실패. 이후 30분마다 반복.
- **09-12 16:34:16**: 마지막 실패. 16:55:45 AdsPower 실행, 16:55:51 LocalAPI 개방, 17:04 크롤링 재개.
- **09-13 00:11**: 정검 중 발견 → ERR-122/ERR-123.
- **09-14 11:35**: AdsPower 헬스체크 경보 운영 반영(`f799ab4`).

**최종 상태:** 감시 경보 운영 중(260914 17:30 실제 DOWN 경보 발송으로 동작 확인). 재부팅 자동복구는 미구현(ERR-130).

**관련:** ERR-122, ERR-123, ERR-130, FP-089

## INC-057 | 260914 재부팅 후 AdsPower Local API 미개방 32분 — 경보로 감지

**기간:** 2026-09-14 17:10 ~ 17:42 (KST)

**운영 영향:** 17:41 크롤링 1회 전체실패. 친구요청은 해당 시간 실행 없음. 게시 영향 없음.

**타임라인:**
- **14:15**: 회장 전원 끄기 → **17:09:18** 부팅, 17:09:30 로그온, 17:09:54 launcher 자동 기동.
- **17:10:15**: AdsPower 자동 실행 → 17:10:18 `ERR_INTERNET_DISCONNECTED`, LocalAPI 미개방.
- **17:10:39~17:30:39**: 헬스체크 3회 실패(`getaddrinfo failed`) → **17:30:39 DOWN Slack 경보**.
- **17:38:53**: 네트워크 연결. AdsPower 자동 재시도 없음.
- **17:41:33**: 크롤링 전체실패 → `[ErrorHandler] fb_crawl` 경보(ERR-123 복구 이후 첫 운영 발생).
- **17:42:38**: 회장 AdsPower 수동 재실행 → 17:42:41 LocalAPI 개방.
- **17:50:39**: 복구 Slack 경보("다운 지속 0시간 40분").

**최종 상태:** 정상 복귀. 원인(네트워크보다 먼저 실행) 기록, 자동복구 설계는 보류(ERR-130). 260915 부팅은 네트워크가 먼저 연결돼 정상.

**관련:** ERR-122, ERR-123, ERR-130

## INC-058 | Live Canary 중 Kill Switch OFF로 게시물 2건 `rejected` 처리

**기간:** 2026-09-15 08:54 ~ 14:05 (KST)

**운영 영향:** 크롤링 신규 게시물 2건이 게시되지 않고 거절 상태가 됨(콘텐츠 결함 아님). 약 5시간 뒤 복구·게시. Kill Switch OFF 동안 친구요청 실행 0회.

**타임라인:**
- **08:54**: 점진 수집 Live Canary 준비 — `IDN-000041 automation_enabled` OFF(read-back), `.env` `FB_PROGRESSIVE_CRAWL_ENABLED=true`.
- **08:57**: 재시작. **08:58~09:03** 크롤링 4그룹 posts 5/8/7/7, 신규 저장 2.
- **09:04:16 / 09:04:20**: `[PublishGate] IDENTITY_REJECTED` → `recChgyTQ6QINI1nX`, `recdnjpk5JQZ1nZ7t` `post_status=rejected`(게시 0).
- **09:1x**: Kill Switch ON 복귀.
- **09:15**: GPT 검수 #2 HOLD → 09:17 `.env` Flag false → 09:19 재기동(점진 수집 OFF 확인).
- **09:30~09:37**: 결함 원인 확정(ERR-129) → 수정·테스트 → 09:37 운영 반영 → `961fc05`.
- **14:04**: 회장 승인으로 2건 `ready` 복구 → 14:04:39·14:05:05 게시 성공(중복 0).

**최종 상태:** Kill Switch 보류 계약 복구 운영 중. 2건 게시 완료.

**관련:** ERR-128, ERR-129, FP-092

## INC-059 | 크롤링 수집 파이프라인 품질·수집량 저하 (장기)

**기간:** 2026-06-04 ~ 2026-09-15 (KST, 각 원인별 상이)

**운영 영향:** (1) 6/4~9/14 OCR 워터마크·연락처 필터가 사실상 작동하지 않아 크롤링 이미지가 무검사로 자동게시 경로에 진입(차단 0건). (2) 7/26 무렵부터 신규 수집이 주 11~15건 → 0~6건으로 급락. (3) 9/12~14 번역 차단으로 관련 게시물 99.9% 제외.

**타임라인:**
- **06-04**: `passes_image_filter` OCR 도입. 운영 venv에 pytesseract 없음 → "OCR 실패 → 통과" 시작(6/11~6/20만 정상 통과 136건).
- **07-26 전후**: 주간 신규 저장 급락(W29 193 → W30 13). 단독 원인 UNKNOWN.
- **09-12~09-14**: Google 번역 `TooManyRequests`로 필터 제외 947/948.
- **09-14**: 원문 키워드 선검사 적용(`c51c842`).
- **09-15**: OCR Fail-closed 운영 반영(`21e77fb`). 숨김 페이지 수집 한계 확인, 점진 수집 코드 Flag OFF로 커밋(`730ef1d`).

**최종 상태:** OCR Gate와 번역 우회는 운영 중. 점진 수집은 재활성화 조건 충족 전까지 OFF. 610113703703488 그룹은 6/17부터 상시 실패(OPEN).

**관련:** ERR-125, ERR-126, ERR-128, FP-090, FP-093

## INC-060 | 배터리 구동 중 Modern Standby 절전 2시간 37분 — 크롤·aijomoojin 슬롯 누락

**기간:** 2026-09-15 17:44:55 ~ 20:21:51 (KST)

**운영 영향:** Facebook 크롤 4회(18:24·18:54·19:24·19:54) 누락, YUNA 친구요청 누락, aijomoojin 콘텐츠 생산 17:00(ICT)·게시 슬롯 18:00(ICT) `[MissedSlot]` 스킵, 깨어난 직후 Airtable DNS 오류(네트워크 미연결). ERR-130 START Canary 스크립트도 절전으로 멈춰 AdsPower 무접촉 ABORT.

**타임라인:**
- **17:44:55**: Modern Standby 진입(배터리 51%, AC 미연결).
- **17:59:56**: 일시 깨어남 후 재진입.
- **20:21:51**: 깨어남 → launcher 누락 경고 후 정상 재개.
- **20:39:42**: 회장 충전기 연결(AcOnline=true).

**최종 상태:** 정상 복귀. 원인은 "24시간 서버는 AC 상시 연결" 운영 원칙 미준수(ERR-114 계열) — 이번 코드 변경과 무관. 이후 21:26·10:35(260916)에도 AC 분리 기록 → 운영 원칙 재확인 필요.

**관련:** ERR-114, FP-085

## INC-061 | launcher 재시작 시점이 크롤·친구요청 실행과 겹쳐 각 1회 중단

**기간:** 2026-09-16 22:23:52 (KST)

**운영 영향:** 22:21:45 크롤 1회 중간 중단, 22:23:45 친구요청 1회 중단("AdsPower 사용 중 — 120초 후 재확인 (1/3)" 대기 중) — 요청 클릭 0, 일일 카운트(3) 무변화, 데이터 손상·중복 발송 증거 없음. 재시작 후 22:25:24 크롤 정상 수행.

**타임라인:**
- **22:21:45**: 크롤 시작(구 launcher).
- **22:23:45**: 친구요청 시작 → AdsPower 사용 중 대기.
- **22:23:52**: 서비스 재시작(권장 시각 22:26 이후보다 이름).
- **22:24:22**: 새 launcher 기동, Airtable 크롤 대상 4건 로드.

**최종 상태:** 정상. 교훈 — 재시작 요청 시 "실행 중 작업 없음"을 확인한 뒤 실행 시각을 명확히 합의한다.
