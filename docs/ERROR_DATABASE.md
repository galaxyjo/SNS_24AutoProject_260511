...
**관련:** `docs/CURRENT_RUNTIME_CONTEXT.md` 260805 01:33 항목의 Gemini 429 quota Blocker와는 별개 원인(그 항목은 ERR-NNN 미부여 상태였다가 ERR-104로 신규 부여됨), INC-047, FP-075

---

## ERR-104 | aijomoojin Gemini 429 — Search Grounding은 Free tier에 요금제 권한 자체가 없음(공식문서 확인), 회장 결정으로 유료 전환 없이 수용 (FREE TIER PLAN RESTRICTION CONFIRMED / ACCEPTED, 260805)

**Type:** Observability / External Provider — `research_to_topic_adapter.py` 실제 Runtime 호출에서 Confirmed

**경위:** aijomoojin 전용 Gemini 프로젝트(`AIJOMOOJIN_GEMINI_API_KEY`)의 429가 260805 하루 3슬롯(09:00·16:00) 및 수동 검증 2회(09:15·16:47) 전부에서 반복됐으나, 기존 로깅은 `category=provider_http_429`만 남기고 원문 예외 본문(quota 종류를 구분할 `quotaMetric`/`quotaId`/`limit`/`retryDelay`)을 전혀 남기지 않았다(로깅 공백). 회장 승인 하에 `_extract_gemini_quota_fields(exc)` 함수를 신규 추가해 HTTP 429일 때 이 4개 Allowlist 필드(+`status_code`)만 안전하게 구조화 로그로 남기도록 최소 수정했다. 합성 테스트(quota 구조 있음/없음/APIError 아님 3종 + Secret 비노출 확인)로 추출 로직 자체가 정상 동작함을 확인한 뒤, 실제 Gemini 호출 1회(17:12:58~17:14:28, 4/4 attempt)로 검증했다.

**Evidence:** `status_code=429`는 매 시도 확인됐으나 `quota_id`·`quota_metric`·`limit`·`retry_delay` **전부 None** — 합성 테스트로 추출 로직 결함이 아님을 먼저 증명했으므로, 이는 **이 프로젝트/키에 대한 실제 Gemini 응답 자체가 QuotaFailure/RetryInfo 구조화 상세를 포함하지 않는다**는 뜻이다. Target Test 29 passed(신규 4개 포함, `tests/test_research_to_topic_adapter.py`), REUSE 대상 `test_generate_hook_caption.py` 25 passed(회귀 0건). `producer_lock` 정상 해제, Airtable Write 0건, 두 번째 호출 0건.

**Risk:** `MEDIUM` — Fail-closed 계약(4회 bounded retry 후 안전 종료, 부분상태 0건)은 그대로 유효해 운영 위험(오게시·중복)으로 이어지지는 않는다 — 콘텐츠 생성이 간헐적 또는 전면적으로 막히는 가용성 문제로 계속 남는다. Free tier를 유지하는 한 이 Blocker는 코드로 해결 불가능한 외부 서비스 제약이다.

**Status(260805 17:57 ICT 갱신, 260805 18:03 ICT 재검수 후 정정, 260805 18:12 ICT GPT 검수 확정):** **PROJECT-SCOPED 429 CONFIRMED / MECHANISM UNKNOWN / ACCEPTED.** "PROJECT-SCOPED"는 260805 17:47~17:49 Key 교체 검증(같은 프로젝트 내 신규 Key로도 동일하게 429 재현)으로 이미 확인된 사실을 명시한 것 — Key 단위 문제가 아니라 프로젝트 단위로 429가 발생함은 Confirmed다. 그 이상의 인과(왜 프로젝트 단위로 발생하는지)는 아래처럼 여전히 UNKNOWN이다. 회장이 직접 Google AI Studio 콘솔(`aistudio.google.com`, 계정 `nguyenknv15@gmail.com`)에 로그인해 Read-only로 확인한 **FACT 2건**은 다음과 같다 — (1) "비율 제한" 페이지에서 실제 호출 모델 **Gemini 3.5 Flash Lite의 개별 RPM(2/15)·TPM(14/250K)·RPD(2/500)는 전혀 소진되지 않은 상태**였다. (2) "지출"·"결제" 페이지에서 **"이 프로젝트에 현재 설정된 결제가 없습니다" / "결제 계정 없음"**을 확인 — 이 프로젝트는 Billing 미연결 Free tier다.

이전 버전(17:57 ICT)은 이 두 FACT로부터 "Billing 미연결이 근본원인일 가능성이 가장 높다"고 결론지었으나, 이는 **과대 추정이었다 — 재검수로 정정**한다. 이유: Free tier 프로젝트는 정의상 전부 결제 미연결이므로, "결제 계정 없음" 자체는 이 프로젝트가 Free tier라는 이미 알려진 사실을 재확인한 것일 뿐, 429가 발생하는 이유를 설명하는 새로운 인과 증거가 아니다. 개별 모델 RPD(2/500)가 전혀 소진되지 않은 상태에서 왜 429가 발생하는지 — Google 측에 모델별 표시 한도와 별도로 존재하는 Free tier 집계 제약이 있는지, 다른 원인(예: 신규 프로젝트 신뢰도 제한 등)인지 — 는 **이번 콘솔 확인으로 증명되지 않았다.** "Billing을 연결하면 해소될 것"이라는 판단 역시 실제로 연결해 검증한 적이 없는 **미검증 가설**이다.

**정정된 결론(260805 18:03 ICT 시점):** FACT는 위 2건뿐이며, 429의 실제 인과 메커니즘은 **UNKNOWN으로 유지**한다. 다만 **회장의 결정("무료로 사용하고 싶다")은 이 인과관계 규명 여부와 무관하게 별개로 유효** — Billing을 연결하지 않기로 확정했으므로, 메커니즘이 무엇이든 이 Blocker를 코드 수정이나 추가 조사로 해결하지 않고 현재 상태(간헐적/전면적 429로 콘텐츠 생성이 자주 막힘)를 **알려진 제약으로 수용(ACCEPTED)**한다. ERR-104의 Observability 코드(`_extract_gemini_quota_fields`)는 그대로 유지 — 향후 429 발생 시에도 `status_code`는 계속 기록된다.

**결정적 신규 Evidence(260805 18:16 ICT, 최소 Raw 요청 검증) — 위 "MECHANISM UNKNOWN"이 크게 좁혀짐:** 회장 지시로 `research_to_topic_adapter._get_client()`(동일 `AIJOMOOJIN_GEMINI_API_KEY`)와 동일 모델(`gemini-3.5-flash-lite`)을 그대로 쓰되, `google_search`/`url_context` Tool과 `response_json_schema` 없이 **완전히 최소화한 Raw 요청**(`contents="ping"`)을 1회 실행했다. 결과: **`HTTP 200 OK`, 응답 `"pong! How can I help you today?"` — 429가 재현되지 않고 성공했다.** 같은 프로젝트·같은 Key·같은 모델인데 도구 없는 호출은 성공하고, 도구(Search Grounding + URL Context)를 포함한 호출만 하루 종일 429였다는 뜻이다. 이는 **"프로젝트 전체가 막혔다"는 가설을 반증**하고, **Search Grounding 또는 URL Context Tool 자체가 이 모델에서 더 낮은 별도 quota를 갖고 있을 가능성**을 직접 시사한다(AI Studio "비율 제한" 페이지의 "도구 > 검색 그라운딩" 행이 base 모델과 별도로 존재했던 것과 정합적 — 다만 그 화면에서 실제로 확인한 건 "Gemini 2.5" 선택 상태였고 "Gemini 3.5"용 도구 quota는 아직 직접 확인하지 않았다는 한계 있음).

**Google 공식 지원 제출 관련(회장 지시 이행 결과):** 원 지시("최소 Raw 요청에서도 429가 재현되면 Google 지원에 제출")의 전제 조건이 **성립하지 않았다** — Raw 요청은 429가 아니라 성공했으므로, 이번 세션에서는 Google 공식 지원에 아무것도 제출하지 않았다(제출 자체가 이 지시의 조건부 Action이었고 조건 미충족).

**Tool 분리 검증 1/2 — Search Grounding만(260805 18:19 ICT, 회장 승인 하 실행):** `google_search` Tool만 포함(`url_context`·`response_json_schema` 제외)한 Raw 요청 1회(`"What is the capital of France?"`) — **즉시 `HTTP 429` 재현**(`RESOURCE_EXHAUSTED`, 동일 메시지 "check your plan and billing details", `details.error.details`에 Help 링크만 있고 QuotaFailure/RetryInfo 없음 — ERR-104 상단에서 이미 확인한 로깅 공백과 일치, 코드 결함 아님). **결론: 최소 1개 조건(base 모델 성공 vs Search Grounding 포함 시 즉시 429)만으로 Search Grounding Tool이 이 429의 실제 병목(또는 병목 중 하나)임이 확정됐다.** URL Context 단독 검증은 이번 라운드에서 지시대로 실행하지 않음(동시검증 금지 준수) — Search Grounding 단독으로 이미 429가 재현됐으므로, URL Context가 추가로 원인인지 여부와 무관하게 **Search Grounding Tool 자체가 확인된 병목**이라는 결론에는 영향 없다.

**최종 판정(260805 18:19 ICT):** MECHANISM **CONFIRMED(부분)** — "Gemini 3.5 Flash Lite + Google Search Grounding Tool" 조합이 이 프로젝트의 Free tier에서 즉시 소진되는 상태다(개별 Search Grounding quota가 base 모델 RPD/RPM/TPM과 별도로 존재하며 그쪽이 막혀 있다는 가설과 정합적, AI Studio 화면에서 "Gemini 3.5" 도구별 quota 수치 자체는 아직 직접 확인 못함 — 완전한 FACT 아닌 강한 정황 증거). aijomoojin Research Adapter는 Topic 리서치에 Search Grounding을 필수로 사용하므로(코드 설계상), 이 Tool이 막혀 있는 한 URL Context 원인 여부와 무관하게 **현재 설계로는 Free tier에서 안정적으로 동작할 수 없다.** 회장의 "무료 유지" 결정(위 정정된 결론 참조)은 그대로 유효 — 이 결과를 근거로 코드 재설계·Billing 연결을 이번 세션에서 진행하지 않는다.

**Tool 분리 검증 2/2 — URL Context만(260805 18:21 ICT, 회장 승인 하 실행):** `url_context` Tool만 포함(`google_search`·`response_json_schema` 제외)한 Raw 요청 1회(`"Summarize https://www.reddit.com/"`) — **`HTTP 429`가 아니라 `HTTP 503 Service Unavailable`** 발생(`"This model is currently experiencing high demand... Please try again later."`, `status=UNAVAILABLE`). 이는 quota 소진 신호가 아니라 **일시적 Provider 과부하** 신호로, Search Grounding 테스트(즉시 429)와는 다른 종류의 실패다. **결론: 이번 1회 시도만으로는 URL Context 자체가 quota 문제를 갖는지 여부를 확정할 수 없다(INCONCLUSIVE)** — 지시된 "재시도 금지"를 그대로 준수해 재호출하지 않았다.

**두 Tool 분리검증 종합:** Search Grounding 단독 = 즉시 429(quota 소진, Confirmed). URL Context 단독 = 503(일시적 과부하, quota 문제인지 INCONCLUSIVE). Base 모델 단독 = 200 성공. 종합하면 **최소 Search Grounding Tool은 이 프로젝트에서 quota가 확실히 막혀 있고, URL Context는 이번 결과만으로는 무죄도 유죄도 아니다.** Research Adapter는 두 Tool을 항상 함께 쓰므로(코드 설계), Search Grounding 하나만 막혀 있어도 전체 파이프라인은 계속 429/실패로 귀결된다 — 이 결론은 URL Context의 상태와 무관하게 유지된다.

**"과거 2.5 성공" 가설 검증 및 반증(260805 18:26~18:28 ICT):** 회장이 "예전 성공 당시엔 2.5 계열이었을 것"이라는 가설을 제기해, 먼저 Read-only로 Commit `778e245`(오늘 01:17:58 ICT, 이 세션 시작 전) 메시지를 확인했다 — 커밋 원문: "`gemini-2.5-flash-lite`/`gemini-2.5-flash` 둘 다 aijomoojin 전용 키 기준 **HTTP 404('no longer available to new users')**로 확인됨(tools 유무 등 7가지 조합 전부 재현), 같은 키로 `gemini-3.5-flash-lite`만 성공 확인" — 즉 2.5는 이 신규 프로젝트에서 quota(429)가 아니라 **애초에 접근 자체가 막힌 404**였고, 그것 때문에 3.5로 교체된 것이었다("과거 2.5 성공"은 사실 아님). 이를 실제로 재검증하기 위해 Search Grounding + `gemini-2.5-flash-lite` 조합으로 Raw 요청 1회를 다시 실행 — **결과: `HTTP 404 NOT_FOUND`, "This model models/gemini-2.5-flash-lite is no longer available to new users"**, 커밋 메시지와 완전히 동일하게 재현됨. **결론: 2.5 계열로 되돌리는 조치는 오늘 아침 이미 고친 404 문제를 재발시킬 뿐, Search Grounding 429는 해결하지 못한다 — 반증 확정.**

**최종 Root Cause 확정(260805 18:35 ICT, GPT 지적을 Claude Code가 공식문서 직접 Fetch로 검증):** GPT가 05/09/16 슬롯 원인 구분 오류(내가 앞선 설명에서 "3슬롯 다 429"처럼 뭉뚱그린 것 — 실제로는 05시=Flag OFF라 미시도, 09시=Sleep으로 Gemini 호출 전에 미실행, **16시(+수동검증들)만 실제로 Search Grounding 429**)와 "Free tier 자체가 Search Grounding을 지원 안 한다"는 주장을 제기해, `ai.google.dev/gemini-api/docs/pricing`과 `ai.google.dev/gemini-api/docs/models/gemini-3.5-flash-lite` 2개 공식 문서를 Claude Code가 직접 Fetch로 재확인했다(GPT 인용을 그대로 믿지 않고 원문 재검증). 결과:
- **Google Search Grounding**: 공식 가격표에 Gemini 3.5/3.6 계열 **Free tier = "Not available"**로 명시(Gemini 2.5 계열만 "1,500 RPD 무료" 별도 허용 — AI Studio 화면에서 본 "4/1.5K"가 왜 "Gemini 2.5" 선택 상태였는지 이걸로 설명됨). 모델 자체 문서(`gemini-3.5-flash-lite`)는 "Search grounding Supported"로 **기술적 지원은 하되 Free tier에 요금제 권한이 없다**는 뜻 — 반증 완료, GPT 주장이 맞다.
- **URL Context**: 같은 가격표에 **Free tier = "Free of charge"**(양쪽 tier 모두 무료, Paid는 토큰만 과금) — Tier 제한이 없다. 이걸로 260805 18:21 URL Context 단독 테스트가 429가 아니라 503(일시적 과부하)이었던 이유가 완전히 설명된다 — URL Context는 애초에 Tier 게이팅 대상이 아니었다.
