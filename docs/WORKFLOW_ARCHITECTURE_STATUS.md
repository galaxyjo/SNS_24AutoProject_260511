...
|--:|---|---|---|---|---|---|
| 1 | Persona Runtime 연결 | **PARTIAL — 코드 배선 완료(260729), Airtable 데이터·Repository lookup은 별도 Gate로 잔존** | 11단계 착수 직전 재개(회장 지시) — `dm_receiver.py`(이미 resolve된 `account_code_ref`를 `handle_price_inquiry()` 호출에 kwarg로 추가)/`dm_auto_reply.py`(`handle_price_inquiry(..., account_code_ref: str = "")` 추가, 로그에 반영)/`ai_reply_generator.py`(`generate_reply(..., tone_style="", greeting_template="", followup_template="")` 추가, 비어있으면 프롬프트 100% 동일) 3파일 수정. 실제 Airtable 조회 로직은 이번 범위에 포함 안 함(schema 확인 결과 `Account_Registry.Persona_Profile`↔`Persona_Profile.account_code_ref` 둘 다 `multipleRecordLinks`인 양방향 Linked Record이며 문자열 lookup 아님 — 계정 2개(IDN-000041/IDN-000036) 전부 링크 0건, `Persona_Profile` 유일 레코드(PER-001)도 tone/greeting/followup 공란이라 지금 조회 로직을 만들어도 검증 불가). `PRICE_AUTO_REPLY_ENABLED=false`라 이번 변경의 Runtime 효과는 0건(YAGNI 그대로 유효) | `Persona_Profile`(persona_role/mbti_type/tone_style/greeting_template/followup_template), 3파일 새 optional 파라미터 | 남은 범위: Airtable Linked Record 조회 함수(Repository 확장, High Risk 재분류 필요) + `Persona_Profile` 콘텐츠(tone/greeting/followup) 실제 입력(회장/콘텐츠 담당) | Repository Interface 변경은 CLAUDE.md 8.2 High Risk 분류 대상 — 착수 전 별도 Gate 6/7 재수행 필요 |
...
- `account_email` 신뢰도: **RESOLVED(260729)** — Runtime 편입 계정 2/2(yuna18253/aijomoojin) 전부 회장 직접 확인으로 CONFIRMED(yuna18253은 260726 기존 확인, aijomoojin은 260729 이번 세션 확인). `modules/` 전체 Grep 결과 이 필드를 참조하는 Runtime 코드는 0건(유일 참조는 `tools/check_account_registry.py`, 존재하지 않는 구식 레코드 대상 죽은 진단 스크립트) — Blast Radius 0 확정. 나머지 31건(`account_code` 미부여, Runtime 미편입 "Ready" 대기 후보)의 email 정확도는 편입 시점에 재검증
