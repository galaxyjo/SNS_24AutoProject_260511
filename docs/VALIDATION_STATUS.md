...
| comment_pipeline_airtable_write_260714 | ERR-062/FP-047/INC-035 — Gate E-A 쓰기 Canary 검증 중 발견·실증: `comment_poller.py` 폴링(11:08)이 테스트 댓글 2건("price plz"/"dm")을 정상 감지했으나, `comment_auto_reply.py`의 `_record_comment()`가 `Lead_Interactions.conversation_channel`에 `instagram_comment` 선택지가 없어 Airtable 기록에 매번 실패(`INVALID_MULTIPLE_CHOICE_OPTIONS`). 예외를 자체 삼키고 반환하는 함수와, 성공 여부 확인 없이 무조건 `processed_comment_ids.json`에 캐시하는 `poll_new_comments()`가 조합되어 실패한 댓글이 영구히 재시도되지 않음(FP-047). `docs/design/DM_RELAY_COMMERCE_RFC.md` "기존 코드 결함(8건)" 1번 항목의 최초 실증. 확인된 손실 2건, 과거 손실 범위는 UNKNOWN. **코드·Airtable 수정 없음 — OPEN.** |
