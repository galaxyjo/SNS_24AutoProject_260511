...
## [260907 추가 — 무인 운영 원칙(UNATTENDED OPERATION PRINCIPLE), 최고 우선순위]

> 근거: 260907 Instagram 좋아요 자동화 Canary 세션 — `IDN-000037`(AdsPower `k1goch3g`) Like 실패 후 Claude Code가 "회장님이 AdsPower에서 Instagram에 직접 로그인해 주세요"라고 요청하자, 회장이 **"이건 24시간 자동화다. 내가 수동으로 뭔가를 하면 안 되는 것이다. 그걸 정확히 명시하고 기록하고 자동화될 수 있게 하는 것이 목적이다"** 라고 명시 확정. 이 원칙은 이후 모든 단계 설계·보고에 적용한다.

### 원칙
**이 시스템의 정상 운영 상태는 "사람이 아무것도 하지 않아도 24시간 돌아가는 것"이다.**
운영 중 사람의 손이 필요한 설계는 그 자체로 **결함(Defect)** 이며, "회장님이 해주세요"는 해결책이 아니라 **미해결 상태의 보고**다.

### 3단계 분류 — 모든 "사람이 해야 하는 일"은 반드시 아래 중 하나로 분류해 보고한다
| 구분 | 정의 | 허용 여부 | 예시 |
|---|---|---|---|
| **A. 운영 중 반복 개입** | 매일·매주 반복해서 사람이 해야 하는 일 | **금지 — 결함으로 취급, 자동화 대상** | 매일 수동 트리거, 주기적 수동 재시작, 세션 만료 시마다 수동 로그인 |
| **B. 계정당 1회 프로비저닝** | 계정을 시스템에 편입할 때 딱 1회만 필요한 일 | 허용(불가피) — 단 **횟수·이유·자동화 불가 근거를 명시**해야 함 | 브라우저 프로필에 최초 1회 Instagram 로그인(비밀번호·2FA 입력) |
| **C. 승인 게이트** | 상태변경 전 회장 승인 | 허용(설계상 의도) | Commit/Push, Airtable Write, Live 실행 승인 |

**B로 분류하려면 다음 3가지를 같은 보고에 반드시 적는다**: ① 계정당 정확히 몇 회인지 ② 왜 자동화할 수 없는지 ③ 그 1회 이후 무인 운영이 유지되는 근거(Runtime Evidence).

### B(1회 프로비저닝)의 현재 확정 항목 — Instagram/Facebook 최초 로그인
- **자동화 불가 근거**: Claude Code는 비밀번호·인증코드 등 Credential을 폼에 대신 입력하지 않는다(안전 규칙, 협상 대상 아님). 2FA·기기 승인·CAPTCHA도 사람 확인이 전제다.
- **1회 이후 무인 근거(Runtime Evidence)**: `k1bto3j4`(YURA/IDN-000041)는 과거 1회 로그인 이후 세션이 AdsPower 프로필에 남아, 260905 Facebook 친구요청 8건/일 무인 운영과 260907 Instagram Like 성공(`recntJtx22uB8O2Hk`)까지 **사람 개입 0회**로 수행됐다.
- 따라서 계정 1개당 사람의 손은 **최초 1회**이며, 그 이후 재로그인이 반복 필요해지면 그것은 A(결함)로 재분류해 원인을 제거한다.

### A(반복 개입)를 없애기 위해 자동화해야 하는 것 — 발견 즉시 이 목록에 추가한다
1. **세션 만료 자동 대응** — 로그인 세션이 끊긴 계정은 (a) 자동 감지 (b) Slack 알림 (c) **그 계정만 Fail-closed 격리**하고 나머지 계정·잡은 계속 돌린다. 사람이 발견할 때까지 조용히 실패하는 구조 금지.
2. **AdsPower 커널/chromedriver 자동 정렬** — `facebook_crawler.py`의 chromedriver 경로 하드코딩(`chrome_144`) 때문에 신규 프로필 커널이 다르면 attach가 깨진다(260907 `k1goch3g` 커널 150 사고). AdsPower `browser/start` 응답의 `data.webdriver` 경로를 그대로 쓰면 프로필별 커널을 자동 추종할 수 있다(GPT 260907 판정: 이것이 장기 정석). **미적용 — 별도 승인 필요.**
3. **프로필 설정 자동 정렬** — 커널 버전 등 프로필 설정은 AdsPower Local API `user/update`로 코드에서 변경 가능하다(260907 실증: `k1goch3g` 150→144 API로 변경 성공). AdsPower UI 수동 조작을 전제로 한 절차는 A(결함)로 본다.

### 보고 형식 규칙
"회장님이 ~해 주세요"를 보고에 쓸 때는 **반드시 위 A/B/C 중 어느 것인지 명시**한다. 분류 없이 수동 작업을 요청하지 않는다. A에 해당하면 요청 대신 **자동화 방안을 제시**한다.
